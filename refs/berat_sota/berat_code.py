# ⚡ [0.341+ SOTA] Enveda CASMI 2026: Quad-Channel Analog Ranker & Transformer
### *Authored by Berat Erol ÇELİK ([@beraterolelk](https://www.kaggle.com/beraterolelk)) — Grandmaster Strategy*

[![Kaggle](https://img.shields.io/badge/Kaggle-Competition-20BEFF?logo=kaggle&logoColor=white)](https://www.kaggle.com/competitions/enveda-CASMI26-molecule-id-mass-spectra)
[![Prize Pool](https://img.shields.io/badge/Prize_Pool-,000_USD-gold.svg)]()
[![Model: Quad--Channel](https://img.shields.io/badge/Model-Quad--Channel_Transformer+GBM-9cf.svg)]()
[![Kinematics: 29--Adducts](https://img.shields.io/badge/Physics-29_High--Precision_Adducts-brightgreen.svg)]()
[![Theme: TokyoNight](https://img.shields.io/badge/Aesthetic-TokyoNight-7aa2f7.svg)]()

---

<div style="border-radius: 12px; background: linear-gradient(135deg, #0d1117 0%, #161b22 100%); border: 1px solid #30363d; padding: 18px 22px; margin-bottom: 20px; box-shadow: 0 8px 24px rgba(0,0,0,0.4);">
  <div style="color: #58a6ff; font-weight: 800; font-size: 15px; margin-bottom: 8px; display: flex; align-items: center; gap: 8px;">
    <span>💡</span> COMPANION KAGGLE BENCHMARKS & COMPETITION PIPELINES
  </div>
  <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px; margin-top: 10px;">
    <div style="background: rgba(30, 41, 59, 0.7); border: 1px solid #3b82f6; border-radius: 8px; padding: 12px;">
      <a href="https://www.kaggle.com/datasets/beraterolelk/kaggle-grandmaster-winning-solutions-2015-2026" style="color: #60a5fa; font-weight: 700; text-decoration: none; font-size: 13px;">📦 Kaggle Grandmaster Winning Solutions (2015–2026)</a>
      <p style="color: #94a3b8; font-size: 11px; margin: 4px 0 0 0;">650+ verified 1st place architectures, feature recipes & writeups.</p>
    </div>
    <div style="background: rgba(30, 41, 59, 0.7); border: 1px solid #10b981; border-radius: 8px; padding: 12px;">
      <a href="https://www.kaggle.com/datasets/beraterolelk/kaggle-tabular-winning-feature-engineering-recipes" style="color: #34d399; font-weight: 700; text-decoration: none; font-size: 13px;">📦 50+ Winning Feature Engineering Recipes</a>
      <p style="color: #94a3b8; font-size: 11px; margin: 4px 0 0 0;">Battle-tested mathematical feature engineering transformations.</p>
    </div>
  </div>
  <div style="margin-top: 12px; font-size: 12px; color: #cbd5e1;">
    ⚡ <i>Top Tabular Baseline:</i> <a href="https://www.kaggle.com/code/beraterolelk/xgboost-lightgbm-orderedte-optuna-0-98088" style="color: #38bdf8; font-weight: 600;">XGBoost + LightGBM | OrderedTE & Optuna [0.98088]</a>
  </div>
</div>

---

### 🧬 Methodological Breakthrough: Quad-Channel Consensus & Adduct Kinematics

Small molecule de novo identification from untargeted tandem mass spectrometry (MS/MS) poses an extreme inverse problem. Standard cosine spectral matching fails on novel natural product scaffolds that do not exist in reference spectral libraries.

This SOTA architecture solves this via **4 orthogonal evidence channels** unified by **Reciprocal Rank Fusion (RRF)**:

1. **High-Precision 29-Adduct Kinematics & timsTOF Calibration:** Eliminates candidate mis-windowing by incorporating 29 adduct forms with exact isotopic masses, applying 0.4$ ppm timsTOF systematic offset compensation.
2. **Zero-Drop Isobaric Pruning:** Resolves the published Class-2 isobar recall bug by retaining all library matches and sorting isobaric candidates strictly by absolute mass residual $|m_{cand} - m_{target}|$.
3. **Channel 1 (Direct Library Entropy & Cosine):** Fast vectorised search against 6,000+ reference spectra.
4. **Channel 2 (Mass-Shifted Analog Propagation):** Quantifies conserved sub-structural motifs across known natural product families using mass-shifted delta spectra.
5. **Channel 3 (MetFrag-Lite Fragmentation Scoring):** In-silico bond-dissociation energy and fragment explainability matching.
6. **Channel 4 (Neural Spectrum-to-Fingerprint Transformer):** Deep FPNet models ( + ) predicting 2,048-bit Morgan fingerprints, ranked via exact Bayes log-likelihood dot products  \cdot z$.
7. **Calibrated 31-Feature Dual-Prior GBM Ensemble + RRF:** 31-dimensional statistical feature space trained across priors  \in \{0.55, 0.65, 0.75\}$ and multiple random seeds, blended with neural Bayes logits via Reciprocal Rank Fusion.
...
class CFG:
    ...
    TOP_KEEP           = 80     # Max candidate pool size per molecule
    CALIBRATE_TIMSTOF  = True   # Compensate for systematic +1.4 ppm timsTOF mass shift
    TIMSTOF_PPM_OFFSET = 1.4    # Systematic offset in parts-per-million
    ...
    ENT_WEIGHT         = True   # Entropy weighting for low-entropy spectra

    # --- Mass-Shifted Analog Propagation (Channel 2) --------------------------------
    ANALOG_WIN         = 200.0  # +- Da mass-shift search window
    N_ANALOG           = 100    # Analogs kept per molecule
    SIM_POWER          = 4.0    # sim^p power weighting. p=4 -> 0.525 Class-2 MRR
    RRF_K              = 40.0   # Reciprocal Rank Fusion smoothing constant

    # --- Calibrated Ensemble Ranker (Variance Reduction) ---------------------------
    W1_PRIORS          = (0.30, 0.60)  # Hedging Class-1/Class-2 trade-off improves LB
    SEEDS              = (0, 1, 2, 3)  # Bagging 4 random seeds
    ...
    USE_BIO_DB         = False  # ChEBI/LIPID MAPS toggle
...
# ===================================================================================
#  Unified Test Inference Pipeline: 4-Channel Retrieval, RRF & Re-ranking
# ===================================================================================
load_neural_models()
...
for gi, (mid, sub) in enumerate(mols):
    ...
    if len(nms):
        ...
        # Apply timsTOF systematic mass calibration if detected
        if CFG.CALIBRATE_TIMSTOF and hasattr(sub, 'instrument'):
            instruments = [str(getattr(r, 'instrument', '')).lower() for r in sub.itertuples()]
            if any('timstof' in ins for ins in instruments):
                target = target * (1.0 - CFG.TIMSTOF_PPM_OFFSET * 1e-6)
        ...
        if len(cand):
            # Zero-Drop Isobaric Pruning (Retain all library hits, fill by exact mass)
            if len(cand) > CFG.TOP_KEEP:
                lv_raw = np.array([lib_hits.get(pool.keys[c], 0.0) for c in cand], np.float32)
                mass_diff = np.abs(pool.mass[cand] - target)
                must_keep = np.where(lv_raw > 0)[0]
                rest = np.where(lv_raw == 0)[0]
                n_fill = max(0, CFG.TOP_KEEP - len(must_keep))
                fill = rest[np.argsort(mass_diff[rest])[:n_fill]] if n_fill else rest[:0]
                keep = np.unique(np.concatenate([must_keep, fill])) if len(fill) else must_keep
                cand = cand[keep]
            ...
            # Neural Bayes log-likelihood score
            if zlog is not None:
                raw_bayes = cfp @ np.asarray(zlog, np.float32)
            else:
                raw_bayes = np.zeros(len(cand), np.float32)

            # Reciprocal Rank Fusion (RRF) with Quad-Channel Precision (Library > Analog > GBM > Neural)
            analog_dict = dict(analogs)
            av = np.array([analog_dict.get(pool.keys[c], 0.0) for c in cand], np.float32)

            r_gbm    = np.argsort(np.argsort(-p_gbm))
            r_bayes  = np.argsort(np.argsort(-raw_bayes))
            r_lib    = np.argsort(np.argsort(-lv))
            r_analog = np.argsort(np.argsort(-av))
            
            K = CFG.RRF_K
            # Fused consensus score: Library certainty (Class 1) > Analog Propagation (Class 2) > GBM Ensemble > Neural Bayes
            rrf_score = (
                (1.0 / (K + r_gbm)) +
                (0.35 / (K + r_bayes)) +
                np.where(lv > 0.40, 3.5 / (15.0 + r_lib), 0.0) +
                np.where(av > 0.40, 2.8 / (20.0 + r_analog), 0.0)
            )

            order = np.argsort(-rrf_score)[:CFG.TOPN]
            smis = [pool.smiles[cand[i]] for i in order]

            diag.append((mid, target, len(cand), float(lv.max()),
                         float(sims[0]) if sims else 0.0, float(p_gbm[order[0]])))
...
print(f"[SUCCESS] submission.csv generated successfully with shape {submission.shape} in {time.time()-T0:.1f}s total.")
submission.head()
...
### 🔥 Related: **[S6E9 Gold Hunt — Closing a 0.00002 AUC Gap](https://www.kaggle.com/code/beraterolelk/s6e9-gold-hunt-closing-a-0-00002-auc-gap)**
*Full post-mortem: digit-power features, rank-space blending, zero-tie lexsort, and the final-day probe protocol.*
