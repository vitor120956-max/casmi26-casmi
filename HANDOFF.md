...
## 2026-09-20 16:55 UTC — ONDA DE SUBMISSÃO AUTOMÁTICA ARMADA (usuário ausente até 18h+ BRT; "não podemos cometer erros")
- `submit_wave.sh` (processo "Onda 21h"): espera 00:00:30 UTC (21:00 BRT) → p/ cada probe (haideptry v1, berat v1, megayak2 v3, seedswap2 v3, priors2 v3): auto-cura (re-baixa output se faltar) → verify_out.sh (FAIL = pula) → checa slots (10 tentativas) → competition_submit_code com msg taggeada PROBE-WAVE1:<tag> → registra em wave_submitted.txt (IDEMPOTENTE, anti-double-submit). Depois: polling de scores até 2h → wave_scores.log.
- SE SANDBOX RESTARTAR ANTES DAS 21h BRT: religar `bash submit_wave.sh` (idempotente). SE RESTARTAR DEPOIS DAS 00:00 UTC: `bash submit_wave.sh now`. wave_submitted.txt diz o que já foi.
- `finish_line.sh` v2 (processo "Linha de chegada v2") continua: baixa+verifica par v3 (~17:15-17:25Z), pusha priors2 v3 com precheck (~18:20Z pronto).
- Frag-harness offline: começar no próximo turno interativo (código novo, sem pressa — disciplina anti-erro). Spec: drafts/derivatives_frag_design.md; harness_data completo.

## 2026-09-20 20:00 UTC — ★ PAR v3 COMPLETE+VERIFICADO; DESCOBERTA: RANK-1 É CONVERGENTE ENTRE TODOS OS ENGINES SÉRIOS
- Restart #5 (~16:47→19:50) matou finish_line+wave; kernels imunes. **seedswap2 v3 + megayak2 v3 COMPLETE**, baixados e **VERIFY PASS** (400 linhas, sem Traceback, pool 712,199). BUG CLI corrigido nos scripts: `kaggle kernels output <ref> -p <dir>` (positional `.` = "unrecognized arguments" no kaggle novo).
- priors2: push LIMPO 19:52:32Z = **versão 2** (tentativa 14:29 bloqueada por GPU não criou versão) → wave corrigida p/ kver=2. `priors_watch.sh` armado (COMPLETE ~20:45Z → baixa+verifica+backup).
- Wave religada ~20:05Z → dispara 00:00:30Z (21:00 BRT). Backups dos 7 CSVs em `wave_backup/` (commitados — proteção anti-restart).
- ★★ ANÁLISE OFFLINE (7 CSVs, 400 ids, auditada contra bug — strings idênticas, 400 rank-1 distintos/arquivo): **TOP-1 IDÊNTICO EM TODOS OS ENGINES SÉRIOS** — base/seedswap2/megayak(blend+canais ours+pv)/haideptry = 400/400; berat = 398/400 (único que move rank-1: m_773a95 amina 2ª↔3ª, m_e8d835 furano re-escrito/isômero). O jogo inteiro mora nos **ranks 2–25**: set25-igual base-vs-seedswap 15.8%, base-vs-mgblend 10.8%, base-vs-mgpv 56.0%, base-vs-mgours 8.5%, base-vs-berat 2.8%, base-vs-haideptry 8.8%, mgpv-vs-mgours 9.0%.
- LEITURA: faixa LB 0.27–0.34 = piso compartilhado de rank-1 (~0.27) + qualidade deep-rank (junks 0.055 quebravam o rank-1). IMPLICAÇÕES: (a) seedswap2 hoje = medida direta do ruído deep-rank de seed no LB; (b) megayak2 trouxe submission_ours.csv + submission_pv.csv separados = matéria-prima de fusão; (c) **alavanca das finais = reordenação 2–25 via fusão cross-engine**, avaliável OFFLINE com labels do train.parquet (harness completo no disco); (d) risco das finais no rank-1 é BAIXO (consenso entre engines).
- Git: re-sync ok (credential order: chmod .bin/gh + gh auth setup-git ANTES de fetch). Próximo turno: após scores da wave (madrugada) → calibrar ruído → desenhar fusão offline.
